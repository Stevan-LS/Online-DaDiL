...
def test_dadil(list_of_datasets, n_samples, n_classes, n_atoms, batch_size):
...
def test_odadil(list_of_datasets, n_samples, n_classes, n_atoms, batch_size):
    results = {'lin':{'wda': 0, 'e':0, 'e_ot':0, 'r':0, 'r_ot':0}, 'rbf':{'wda': 0, 'e':0, 'e_ot':0, 'r':0, 'r_ot':0}, 'RF':{'wda': 0, 'e':0, 'e_ot':0, 'r':0, 'r_ot':0}}
    n_datasets = len(list_of_datasets)
    for j in range(n_datasets):
        print(f'{j+1}/{n_datasets}')
...
def test_forgetting_odadil(list_of_datasets, n_samples, n_classes, n_atoms, batch_size):
    results = {'lin':{'wda': 0, 'e':0, 'e_ot':0, 'r':0, 'r_ot':0}, 
               'rbf':{'wda': 0, 'e':0, 'e_ot':0, 'r':0, 'r_ot':0}, 
               'RF':{'wda': 0, 'e':0, 'e_ot':0, 'r':0, 'r_ot':0}}
    ...
    for j in range(n_datasets):
        ...
        if j == 0:
            ...
            XP = [XPk.detach().clone() for XPk in dictionary_sources.XP]
            YP = [YPk.detach().clone().softmax(dim=-1) for YPk in dictionary_sources.YP]

            # Test classif sources avant OGMM
            for i in range(len(weights_list)):
                Xr, Yr = dictionary_sources.reconstruct(weights=weights_list[j])
        ...
        if j == 0:
            weights_list = dictionary_target.A[:-1, :].detach()

            # Test classif sources après online learning
            for i in range(len(weights_list)):
                Xr, Yr = dictionary_target.reconstruct(weights=weights_list[j])
    ...
    return results, before_online_results, after_online_results